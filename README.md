...
职业规划聊天会把长期画像、项目历史、近期记录和滚动摘要组合进 prompt。长会话触发四级水位线压缩时，只压缩即将发送给模型的上下文，SQLite 里的完整聊天历史不会被删除。
...
**职业规划聊天** - 结合 `SOUL.md`、能力状态、项目历史和近期记录，帮你整理空余时间、能力积累和作品方向；长会话会用水位线策略保留摘要和最近消息。
...
长会话测试或本地低上下文模型可按需降低 `DAYPILOT_CONTEXT_LIMIT_TOKENS`；默认值是 `64000`。
...
Career Chat 额外维护一份 `career_chat_memory_summaries` 滚动摘要表，用来承接长会话里的旧决策、偏好、约束和待办；消息表仍作为完整历史来源。
...
python backend\tests\test_context_waterline_service.py
