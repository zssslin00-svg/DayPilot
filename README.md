...
启动脚本会自动完成这些事：在真实 LLM 模式下检查 `DEEPSEEK_API_KEY`、备份已有数据库、首次运行时初始化 SQLite、启动后端 `http://127.0.0.1:8000`、启动前端 `http://127.0.0.1:5173/pages/index.html`，并打开浏览器。使用 `--restart` 时会先停止旧的 DayPilot 进程，避免端口占用、旧静态资源缓存或重复服务导致页面不是最新版本。
