...
**自然语言项目更新**：用自然语言添加项目、标记项目完成、更新项目状态，或直接编辑 `SOUL.md` 的“当前项目”段落；打开或刷新 Today 页时系统会自动同步当前项目列表、进度和目标，并把项目事实用于后续目标与复盘。
...
| 项目变化 | 网页左侧 **项目更新**，或直接编辑 `SOUL.md` 的“当前项目”段落 | “新增项目：... 当前进度：... 目标：...”，或维护 `P0 项目名：当前进度：...。目标：...。` 这种单行项目列表 | 写入 SQLite 项目表，并更新 `SOUL.md` 的当前项目段落；打开或刷新 Today 页时会先自动导入 SOUL，从该列表消失的 active 项目会标记完成并保留历史。 |
...
1. P0 项目名：当前进度：当前阶段、最近阻塞。目标：希望今天推进到什么程度。
...
> **重要：启动前先编辑 `SOUL.md` 的“当前项目”和 `.env` 的 API Key。首次打开 Today 页会自动导入 SOUL；运行中修改 SOUL 后，也可以点击「刷新」立即同步。**
...
- 你可以在网页里更新项目，也可以直接改 `SOUL.md` 的“当前项目”。打开或刷新 Today 页时，新项目会自动导入，改名、进度、目标和优先级会同步；从列表里移走的项目会标记完成，不会删掉历史。
- Check-in 推断出的最新项目进度会写回 `SOUL.md` 的当前项目段落；如果写回失败，会进入本地 retry queue，由后台维护循环重试。
