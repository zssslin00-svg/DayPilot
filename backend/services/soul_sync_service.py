...
def enqueue_soul_sync_retry(
    ...
    *,
    ...
    try:
        with connection:
            return repo.create_soul_sync_retry_job(
                connection,
                job_type=job_type,
                status="pending",
...
def _retry_one_job(db_path: str | Path, job: dict[str, Any], *, soul_path: Path) -> dict[str, Any]:
    ...
    try:
        backup_path = _run_job(db_path, job, soul_path=soul_path)
    ...
    payload["last_soul_backup_path"] = str(backup_path)
    _update_job(
        db_path,
        job_id,
        status="succeeded",
    ...
    return {"id": job_id, "status": "succeeded", "soul_backup_path": str(backup_path)}


def _run_job(db_path: str | Path, job: dict[str, Any], *, soul_path: Path) -> Path:
    job_type = str(job.get("job_type") or "")
    if job_type == "profile_memory":
        from backend.services.profile_memory_service import sync_profile_memory_to_soul

        return sync_profile_memory_to_soul(db_path, soul_path=soul_path)
    if job_type == "project_lifecycle":
        from backend.services.project_lifecycle_service import sync_current_projects_to_soul

        return sync_current_projects_to_soul(db_path, soul_path=soul_path)
    raise ValueError(f"unsupported_soul_sync_job_type:{job_type}")
